...
void *glove_thread(void *vid) {
    ...
    long long thread_lines_train;
    long long thread_lines_total;
    CREC cr;
    ...
    thread_lines_train = (long long)(lines_per_thread[id] * train_percentage);
    thread_lines_total = lines_per_thread[id];

    for(a = 0; a < thread_lines_train; a++) {
        fread(&cr, sizeof(CREC), 1, fin);
        if(feof(fin)) break;
        
        if (sample > 0) {
            real r = (sample * train_words) / word_cnt[cr.word1];
            real keep = sqrt(r) + r;
            if(keep < 1) cr.val *= keep;
        }
    ...
    for(a = thread_lines_train; a < thread_lines_total; a++) {
        fread(&cr, sizeof(CREC), 1, fin);
        if(feof(fin)) break;
        ...
        fdiff = (cr.val > x_max) ? diff : pow(cr.val / x_max, alpha) * diff; // multiply weighting function (f) with diff
        //fdiff = diff;
        cost_valid[id] += 0.5 * fdiff * diff; // weighted squared error
    }
...
int train_glove() {
    ...
    strcpy(tfile, save_W_file);
    
    for(b = 0; b < num_iter; b++) {
        total_cost_valid = total_cost = 0;
        ...
        for (a = 0; a < num_threads; a++) total_cost_valid += cost_valid[a];
        fprintf(stdout,"iter: %03d  cost: %lf  valid: %lf\n", b+1, 
            total_cost/(num_lines*train_percentage), 
            total_cost_valid/(num_lines*(1-train_percentage)));
        fflush(stdout);
        sprintf(save_W_file, "%s_%d", tfile, b+1);
        //if(b % 10 == 9)
            save_params();
    }
    //strcpy(save_W_file, tfile);
    return save_params();
}
...
int main(int argc, char **argv) {
    ...
    cost_valid = malloc(sizeof(real) * num_threads);
    ...
    train_words = 0;
    word_cnt = (long long *)malloc(sizeof(long long) * (vocab_size+1)); // frequency for each word
    fid = fopen(vocab_file, "r");
    sprintf(format,"%%%ds %%lld", MAX_STRING_LENGTH); // Format to read from vocab file, which has frequency data
